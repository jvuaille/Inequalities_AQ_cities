...
def flag_above_median_next_gap_v2(
    ...
    ----------
    df : pd.DataFrame
        One row per cluster with a unique identifier and variable columns.
    cluster_col : str
        Column with the unique cluster identifier.
    pollutant_cols : list[str]
        Names of pollutant variables (e.g., MEAN_pm_25, MEAN_pm_10, MEAN_no_2, MEAN_o3_p932).
    socio_cols : list[str]
        Names of socio-economic variables (e.g., MEAN_Y_LT15_share, ..., MEAN_RASTERVALU).
    value_cols : list[str], optional
        Variables to analyze (defaults to pollutant_cols + socio_cols).
    pct_threshold : float
        Fractional threshold (0.05 = 5%).
    eps : float
    ...
    -------
    dict with:
      - data_t : DataFrame (variables × clusters) of values
      - medians : Series of per-variable medians
      - above_median_flags_t : DataFrame (variables × clusters) booleans for x > median
      - next_gap_flags_t : DataFrame (variables × clusters) booleans (this rule)
      - rel_gap_to_next_t : DataFrame (variables × clusters) of computed relative gaps
      - any_5pct_flag_pollution_per_cluster : Series (bool)
      - any_5pct_flag_socio_per_cluster : Series (bool)
      - five_pct_flag_both_types_per_cluster : Series (bool)
    """
    ...
    for var in data_t.index:
       ...
       # ---------- Employment-specific LOW-side rule (UPDATED as requested) ----------
...
# EXAMPLE USAGE
# -------------------------
path =r'C:\Users\Vuaille\OneDrive - European Environment Agency\Unequal Exposure Unequal Impact 2\Results\2 PCA\PCA_052025\EU_wide'
version='v3'
for batch_nb in [1,2,3]:
    ...
    df_full_ind.to_csv(path+'\\'+"AQ_census_2021_EU_PCA_clusters_batch%d_with_indicators_%s.csv"%(batch_nb, version), index=False)
